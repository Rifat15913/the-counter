package com.itechsoftsolutions.mtcore.utils.helper.imagepicker
...
import com.itechsoftsolutions.mtcore.utils.helper.FileUtils
...
object ImagePickerUtils {
    ...
    const val ONLY_CAMERA = 1
    const val ONLY_GALLERY = 2
     ...
     * @param flag to know if we will use only camera, gallery or both
     * */
    @Synchronized
    fun pickImage(activity: Activity, listener: Listener, flag: Int) {
        if (PermissionUtils.isAllowed(Manifest.permission.CAMERA)
            ...
            val imagePickingIntent = getImagePickingIntent(activity, flag)
     ...
     * */
    @Synchronized
    fun pickImageAndCrop(activity: Activity, listener: ImageCropperUtils.Listener, flag: Int) {
        if (PermissionUtils.isAllowed(Manifest.permission.CAMERA)
                && PermissionUtils.isAllowed(Manifest.permission.READ_EXTERNAL_STORAGE)
                && PermissionUtils.isAllowed(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
     ...
     * */
    @Synchronized
    fun pickImageAndCrop(fragment: Fragment, listener: ImageCropperUtils.Listener, flag: Int) {
        if (PermissionUtils.isAllowed(Manifest.permission.CAMERA)
                && PermissionUtils.isAllowed(Manifest.permission.READ_EXTERNAL_STORAGE)
                && PermissionUtils.isAllowed(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            ...
            if (fragment.context != null) {
                val imagePickingIntent = getImagePickingIntent(fragment.context!!, flag)
     ...
     * */
    @Synchronized
    fun pickImage(fragment: Fragment, listener: Listener, flag: Int) {
        if (PermissionUtils.isAllowed(Manifest.permission.CAMERA)
                && PermissionUtils.isAllowed(Manifest.permission.READ_EXTERNAL_STORAGE)
                && PermissionUtils.isAllowed(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            mImagePickingListener = listener
    ...
    private fun getImagePickingIntent(context: Context, flag: Int): Intent? {
