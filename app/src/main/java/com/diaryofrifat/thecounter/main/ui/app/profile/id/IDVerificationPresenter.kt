...
class IDVerificationPresenter : BasePresenter<IDVerificationMvpView>() {

    companion object {
        const val TYPE_FRONT = "type_front"
        const val TYPE_BACK = "type_back"
    }

    var imageFileMap: HashMap<String, File> = HashMap()

    fun uploadUserID(context: Context, idType: Int, imageFileMap: HashMap<String, File>) {
        val dialog = ProgressDialogUtils.on().showProgressDialog(context)

        compositeDisposable.add(
            when (idType) {
                IDVerificationFragment.TYPE_PASSPORT -> {
                    BaseRepository.on().uploadPassport(
                        Compressor(context).compressToFile(imageFileMap[TYPE_FRONT])
                    )
                }

                IDVerificationFragment.TYPE_NID -> {
                    BaseRepository.on().uploadNationalID(
                        Compressor(context).compressToFile(imageFileMap[TYPE_FRONT]),
                        Compressor(context).compressToFile(imageFileMap[TYPE_BACK])
                    )
                }

                else -> {
                    BaseRepository.on().uploadUtilityBill(
                        Compressor(context).compressToFile(imageFileMap[TYPE_FRONT])
                    )
                }
                ...
                .subscribe({
                    ...
                    if (it.code() == HttpURLConnection.HTTP_UNAUTHORIZED) {
                        ...
                        if (response == null) {
                            mvpView?.onError(DataUtils.getString(R.string.profile_activity_error_could_not_upload))
                        } else {

                            getUserFromCloud(context)

                            if (response.isSuccessful) {
                                mvpView?.onSuccess(response.message, idType)
                            } else {
