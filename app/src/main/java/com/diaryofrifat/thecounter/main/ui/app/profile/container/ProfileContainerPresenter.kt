...
class ProfileContainerPresenter : BasePresenter<ProfileContainerMvpView>() {
    fun getProfileBadges(context: Context) {
        // val dialog = ProgressDialogUtils.on().showProgressDialog(context)

        compositeDisposable.add(
            BaseRepository.on().getProfileBadges()
                    ...
                    if (it.code() == HttpURLConnection.HTTP_UNAUTHORIZED) {
                        ...
                        if (response == null) {
                            ...
                            if (response.isSuccessful) {
                                val list: MutableList<ProfileBadge> = ArrayList()

                                if (response.data
                                        .asJsonObject[Constants.JsonKeys.BADGE]
                                        .isJsonArray
                                    && !(response.data
                                        .asJsonObject[Constants.JsonKeys.BADGE]
                                        .isJsonNull)
                                    ...
                                    response.data
                                        ...
                                        .forEach { jsonElement ->
                                            list.add(
                                                ProfileBadge(
                                                    jsonElement
                                                        .asJsonObject[Constants.JsonKeys.NAME]
                                                        .asString,
                                                    jsonElement
                                                        .asJsonObject[Constants.JsonKeys.IMAGE]
                                                        .asString
                                                )
                                            )
                                        }
                                ...
                                mvpView?.onGettingProfileBadgeList(list)
                            } else {
    ...
    fun uploadProfilePicture(context: Context, imageFile: File) {
        ...
        compositeDisposable.add(
            BaseRepository.on().uploadProfilePicture(Compressor(context).compressToFile(imageFile))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe({
                    dialog?.dismiss()
                    if (it.code() == HttpURLConnection.HTTP_UNAUTHORIZED) {
                        ...
                        if (response == null) {
                            mvpView?.onError(DataUtils.getString(R.string.profile_activity_error_could_not_upload_profile_picture))
                        } else {
                            if (response.isSuccessful) {
                                if (response.data
                                        .asJsonObject[Constants.JsonKeys.IMAGE]
                                    ...
                                    && !(response.data
                                        .asJsonObject[Constants.JsonKeys.IMAGE]
                                        .asJsonObject[Constants.JsonKeys.PHOTO]
                                        .isJsonNull)
                                ) {
                                    mvpView?.onUploadingProfilePicture(
                                        response.data
                                            .asJsonObject[Constants.JsonKeys.IMAGE]
                                            .asJsonObject[Constants.JsonKeys.PHOTO]
                                            .asString
